...
class PDG2SeqCell(nn.Module):  #这个模块只进行GRU内部的更新，所以需要修改的是AGCN里面的东西
    ...
    def forward(self, x, state, node_embeddings):
        ...
        # node_embeddings: tuple, mỗi cái [B, N, embedding_dim]
        B, T, N, D = x.shape
        outputs = []
        h = state
        for t in range(T):
            xt = x[:, t, :, :]  # [B, N, D]
            h = self._forward_one_step(xt, h, node_embeddings)
            outputs.append(h.unsqueeze(1))  # [B, 1, N, hidden_dim]
        outputs = torch.cat(outputs, dim=1)  # [B, T, N, hidden_dim]
        return outputs

    def _forward_one_step(self, x, state, node_embeddings):
